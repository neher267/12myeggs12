		...
		<div class="forms">
			<div class="row">
				<div class="col-md-12">
					...
					<a href="{{route('mix-products.images.create', $product)}}" class="btn btn-default">Add New Image</a>		
					...
					<table class="table table-striped table-bordered datatable" cellspacing="0"	>
						...
						@foreach($images as $image)
							<tr>
								...
								<!-- <td>
									<a href="{{route('mix-products.images.edit', [$product, $image])}}" class="btn btn-default">Edit</a>

									<form action="{{route('mix-products.images.destroy',[$product, $image])}}" method="POST" style="display: inline;">
										{{ csrf_field() }}
										{{ method_field('DELETE') }}
								...
								<td>									
									@if($image->type == "Thumbnail")
										@if($product->thumbnail == $image->src)
											<form action="{{route('mix-products.images.update',[$product, $image])}}" method="POST" style="display: inline;">
												{{ csrf_field() }}
												{{ method_field('PUT') }}
